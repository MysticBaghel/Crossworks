    ...
    <section className="bg-dark-secondary py-24 px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div key={current} className="relative bg-dark-card border border-subtle rounded-2xl min-h-[280px] p-12 flex flex-col justify-end overflow-hidden animate-fade-in opacity-0 [animation-fill-mode:forwards]">
          <div className="absolute left-0 top-8 bottom-8 w-1 bg-blue rounded-r-sm" />
        ...
        <div className="flex items-center justify-between mt-6">
          <div className="flex gap-3">
            <button onClick={prev} className="w-11 h-11 rounded-full border border-white/20 hover:border-blue hover:text-blue text-white flex items-center justify-center bg-transparent cursor-pointer transition-all">
              <ChevronLeft size={18} />
            </button>
            <button onClick={next} className="w-11 h-11 rounded-full border border-white/20 hover:border-blue hover:text-blue text-white flex items-center justify-center bg-transparent cursor-pointer transition-all">
              <ChevronRight size={18} />
            </button>
          </div>
          <div className="flex items-center gap-2 text-slate-text text-sm">
            <span className="w-9 h-1 bg-blue rounded-full inline-block" />
            {current + 1}/{portfolioItems.length}
          </div>
        </div>

        <div className="text-center mt-12">
          <a href="#" className="inline-flex items-center gap-3 px-10 py-4 rounded hover:scale-110  border border-white/20 hover:border-blue text-white font-bold transition-all no-underline">
